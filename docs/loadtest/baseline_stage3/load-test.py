...
print("Starting load test sequence...")
...
# 1. Error Rate Plot
plt.figure()
plt.plot(rps, [r[6] for r in results], marker="o", color="crimson", linewidth=2)
plt.xlabel("Offered RPS")
plt.ylabel("Error rate (%)")
plt.title("Breaking Point Analysis")
...
plt.title("RPS vs Response Latency")
plt.grid(True, linestyle="--", alpha=0.6)
plt.legend()
plt.savefig(os.path.join(DIR, "latency.png"), dpi=150, bbox_inches="tight")

print("Plots saved successfully under docs/loadtestGem/")
